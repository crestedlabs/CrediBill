...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Your App      │    │   CrediBill     │    │ Payment Provider│
│                 │    │                 │    │                 │
│ • Payment UI    │◄──►│ • Subscriptions │◄──►│ • Flutterwave   │
│ • Webhook recv  │    │ • Invoices      │    │ • PawaPay       │
│ • User mgmt     │    │ • Webhooks      │    │ • Pesapal       │
│                 │    │ • Tracking      │    │ • DPO           │
└─────────────────┘    └─────────────────┘    └─────────────────┘
```
...
const webhookResponse = await fetch('https://api.credibill.com/v1/apps/webhooks', {
  method: 'PUT',
  headers: {
    'Authorization': `Bearer ${CREDIBILL_API_KEY}`,
    'Content-Type': 'application/json',
...
**PawaPay:**
- Dashboard → API Configuration → Webhooks  
- Add: `https://giant-goldfish-922.convex.site/webhooks/pawapay`

**Pesapal:**
- Dashboard → Developer → IPN Settings
- Add: `https://giant-goldfish-922.convex.site/webhooks/pesapal`

**DPO Pay:**
- Dashboard → Payment Options → Callback URLs
- Add: `https://giant-goldfish-922.convex.site/webhooks/dpo`

> **⚠️ Important:** Use your **actual payment credentials** when making payments so the provider can authenticate and send webhooks to CrediBill.
...
async function createSubscription(userEmail: string, planId: string) {
  ...
  const customerResponse = await fetch('https://api.credibill.com/v1/customers', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${CREDIBILL_API_KEY}`,
      'Content-Type': 'application/json',
  ...
  const subscriptionResponse = await fetch('https://api.credibill.com/v1/subscriptions', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${CREDIBILL_API_KEY}`,
      'Content-Type': 'application/json',
...
app.post('/payment/callback/flutterwave', async (req, res) => {
  ...
  if (status === 'successful') {
    ...
    const verification = await fetch(`https://api.flutterwave.com/v3/transactions/${transaction_id}/verify`, {
...
```
1. User signs up → subscription.created (status: "trialing")
2. Trial expires → subscription.trial_expired
3. User pays → subscription.activated (status: "active")
4. Monthly renewal → subscription.renewed + payment.due
```

### Failed Payment Recovery
```
1. Payment fails → payment.failed
2. After 3 failures → subscription.past_due
3. User updates payment → subscription.activated
```

### Plan Changes
```
1. User upgrades → subscription.plan_changed
...
**Remember**: CrediBill tracks subscriptions, you handle payments. This separation keeps things simple and secure! 🚀